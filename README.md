...
| 緯度経度 | 階ごとに設定できる（右パネル LOCATION）。保存 JSON に入るのでダウンロードしたファイルにも付いてくる。通報の `info` にも乗る |
...
- その最初の 1F にだけ見本の図面（`cocololo.jpg`）を入れる。白紙だとカメラを
  置く場所が無いため。利用者が足す建物・階は白紙から始まる。**既にある階には
  後から入らない**（入るのはストレージが空のときに作る最初の1枚だけ）。
  見本が取れなくても（`file://`、オフライン、配り忘れ）白紙で続行する
...
階ごとに1組。**書類の一部**なので保存 JSON に入り、ダウンロードしたファイルにも
付いてくる。ストレージが無い単体動作でも使える。

```json
{"app":"camplan","version":1,"marker":16,"lat":35.6812345,"lon":139.7671248,
 "walls":[],"cameras":[...]}
```

| | |
|---|---|
| C++ | `Document::lat` / `lon`（`std::optional<double>`、度、WGS84） |
| WASM | `cp_get_lat` / `cp_get_lon` / `cp_set_lat` / `cp_set_lon` |
| 未設定 | **NaN**。get が NaN を返し、set に NaN を渡すと未設定に戻る |

- `float` ではなく `double`。float の緯度は約1m に丸まる。書き出しは `%.7f`
  （0.0000001度 ≒ 1cm）で、末尾の 0 は落とす
- 範囲外（緯度 ±90、経度 ±180 の外）は捨てる。`lat` / `lon` が無い古い保存
  ファイルもそのまま読める
- **undo では戻らない。** undo が見るのは描いたもの（カメラ・壁・マーカー）で、
  打ち込んだ座標を Ctrl+Z が取り消すのは誰も期待しない
